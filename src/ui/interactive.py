...
def interactive_mode(user_data, problem_set, args, all_problems, problem_to_companies, my_companies, topics):
    ...
    if len(problems) == 0:
        print("You have completed all problems in the selected set. Re-picking from entire problem set")
        problems = pick_problems(
            user_data=user_data,
            all_problems=all_problems,
    ...
    for (idx, leetcode_id) in enumerate(problems):
        ...
        while True:
            ...
            if inp == 'hint':
                ...
                print_info(all_problems, problem_to_companies, my_companies, leetcode_id)
            elif inp == 'pause':
                pause_time = timer()
                ...
                print("Available commands: info, hint, easy, hard, quit, pause, break, skip, revisit, refresh")
            else:
