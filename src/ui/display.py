def print_info(all_problems, problem_to_companies, my_companies, leetcode_id):
    ...
    print(
        f"{leetcode_id} {problem['Name']} is {difficulty_string}: {problem['Acceptance']} of submissions pass"
    )
    company_list = my_companies & set(problem_to_companies[str(leetcode_id)])
    company_list_string = (
        f"including: {', '.join(company_list)}"
        if len(company_list) > 0
        else f"including: {','.join(problem_to_companies[str(leetcode_id)][:5])}"
    )
    print(
        f"{len(company_list)} companies have asked this question {company_list_string}"
    )
...
def show_helper_menu(topics):
    ...
    if topic_indices.strip() and topic_indices.lower() != "all":
        try:
            ...
            selected_topics = [
                topic_keys[idx - 1] for idx in indices if 0 < idx <= len(topic_keys)
            ]
        except ValueError:
    ...
    print("1. Easy\n2. Medium\n3. Hard")
    diff_input = input(
        "Enter difficulty numbers (comma-separated), leave blank for all: "
    )
    selected_difficulty = []
    if diff_input.strip():
    ...
    # 6. Interactive mode
    interactive = (
        input("\nDo you want to use interactive mode? (y/n, default: n): ")
        .lower()
        .startswith("y")
    )
